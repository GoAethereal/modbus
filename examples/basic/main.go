...
func server() {
	...
	s.Serve(ctx, &modbus.Mux{
		...
		ReadCoils: func(_ cancel.Context, _ byte, _, quantity uint16) (res []bool, ex modbus.Exception) {
		...
		WriteMultipleRegisters: func(_ cancel.Context, _ byte, _ uint16, values []byte) (ex modbus.Exception) {
...
func client() {
	...
	for i := 0; i < 10; i++ {
		...
		res, err := c.ReadCoils(ctx, 1, 0, 10)
		if err != nil {
			fmt.Println(err)
			return
		}
		...
		if err := c.WriteMultipleRegisters(ctx, 1, 0, []byte("hello there!")); err != nil {
			fmt.Println(err)
			return
		}
		time.Sleep(1 * time.Second)
